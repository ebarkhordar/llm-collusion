...
def build_pairs(records: Iterator[Dict[str, Any]], target_model: str) -> List[Pair]:
    """
    ...
    for (benchmark, task_id), items in grouped.items():
        ...
        # Skip if we don't have target model's code or no other models
...
def execute(
    ...
    pairs = build_pairs(records, target_model)
    if not pairs:
